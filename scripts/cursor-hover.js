...
function funMouse() {
  ...
  const elements = [
    ...document.querySelectorAll("#techstack .list__item .img-wrapper, a"),
  ];
  ...
  var mouseInit = function mouseInit() {
    ...
    function on(el) {
      ...
      canvasContext.strokeStyle = "#ebf4f3";
      canvasContext.fillStyle = "#ebf4f3";

      let text = el.getAttribute("data-text");
      if(text){
        text = text.toUpperCase();
        const textWidth = canvasContext.measureText(text).width;
        const textHeight = 16; // Assuming 16px font size

        // Draw text centered on the circle
        canvasContext.fillText(
        text,
        circle.lastX - textWidth / 2,
        circle.lastY + textHeight / 4
        );
      }
      canvasContext.stroke();
      canvasContext.closePath();

      requestAnimationFrame(()=> on(el));
    }
